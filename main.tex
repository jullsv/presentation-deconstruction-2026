...
\framesubtitle{Реализация автоматизированной деконструкции}

Этапы обработки текста:

\begin{itemize}
\item{Загрузка текста из файла (PDF/TXT);}
\item{Извлечение ключевых утверждений с~помощью промптов;}
...
\item{Генерация структурированного отчёта.}
\end{itemize}

{\bf Технологический стек:}

\begin{itemize}
\item{Язык реализации: Python 3;}
\item{Языковая модель: YandexGPT;}
\item{Формат результатов: JSON / TXT.}
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Примеры промптов}
\framesubtitle{Техника подсказок для каждого этапа}

{\small

{\bf Этап 1: Извлечение утверждений}

\textit{«Найди 2–3 самые важные мысли автора. Напиши каждую мысль отдельной строкой.»}

\vspace{0.3cm}

{\bf Этап 2: Бинарные оппозиции}

\textit{«В тексте есть два слова, которые спорят друг с~другом. Одно слово автор любит больше, второе — меньше. Найди эту пару слов.»}

\vspace{0.3cm}

{\bf Этап 3: Инверсия}

\textit{«Автор думает, что \{A\} важнее \{B\}. Но давай посмотрим наоборот: а~что, если \{B\} на~самом деле помогает \{A\} быть?»}

\vspace{0.3cm}

{\bf Этап 4: Поиск ограничений}

\textit{«Найди в~тексте фразы, которые показывают: автор не~учитывает что-то важное.»}
